...
public:
	...
	tControl ResetMsb;
	tControl ResetLsb;
	tParam( long clk, int cha, uchar id1, uchar msb, uchar id2, uchar lsb, uchar msbval) :
	   Msb( clk, cha, id1, msb ),
	   Lsb( clk, cha, id2, lsb ),
	...
	virtual int GetVal() { return( DataMsb.Value ); }
};

class tNrpn : public tParam {
public:
	tNrpn( long clk, int cha, uchar msb, uchar lsb, uchar msbval ) :
...
public:
	tRpn( long clk, int cha, uchar msb, uchar lsb, uchar msbval ) :
...
class tUndoBuffer : public tSimpleEventArray
{
  ...
  public:
    virtual void Clear();
    void Put(tEvent *e, int killed) {
      bits.set(nEvents, killed);
      tSimpleEventArray::Put(e);
    }
  private:
    tBitset bits;  // set for killed events
};
