...
// Modified by:
// Created:     04/01/98
// RCS-ID:      $Id: proplist.cpp 38409 2006-03-28 11:02:37Z ABX $
...
bool wxPropertyListView::UpdatePropertyList(bool clearEditArea)
{
  ...
  for (
    map<wxString, wxProperty*>::const_iterator iProperty =
      m_propertySheet->GetProperties().begin();
    iProperty != m_propertySheet->GetProperties().end();
    ++iProperty)
  {
    wxProperty* pProperty = iProperty->second;
    wxString stringValueRepr(pProperty->GetValue().GetStringRepresentation());
    wxString paddedString(MakeNameValueString(pProperty->GetName(), stringValueRepr));
    m_propertyScrollingList->Append(paddedString.GetData(), (void *)pProperty);
  }
  return true;
}
...
bool wxPropertyListValidator::OnValueListSelect(
  wxProperty *property,
...
bool wxPropertyListValidator::OnDisplayValue(
  wxProperty *property,
  wxPropertyListView *view,
  wxWindow *parentWindow)
{
...
bool wxPropertyListValidator::OnRetrieveValue(
  wxProperty *property,
  wxPropertyListView *view,
  wxWindow *parentWindow)
{
...
void wxPropertyListValidator::OnEdit(
  wxProperty *property,
  wxPropertyListView *view,
  wxWindow *parentWindow)
{
...
bool wxPropertyListValidator::OnClearControls(
  wxProperty* property,
  wxPropertyListView *view,
  wxWindow *parentWindow)
{
...
///
bool wxRealListValidator::OnCheckValue(
  wxProperty *property,
  wxPropertyListView *view,
  wxWindow *parentWindow)
{
...
bool wxListOfStringsListValidator::OnPrepareControls(
...
// Called when the property is double clicked. Extra functionality can be
// rovided, cycling through possible values.
bool wxListOfStringsListValidator::OnDoubleClick(
  wxProperty* property,
  wxPropertyListView *view,
  wxWindow *parentWindow)
{
...
void wxListOfStringsListValidator::OnEdit(
  wxProperty *property,
  wxPropertyListView *view,
  wxWindow *parentWindow )
{
  // Convert property value to a list of strings for editing
  list<wxString>* pStringList = new list<wxString>;
  ...
  if (EditStringList(parentWindow, pStringList, title.GetData()))
  {
    wxPropertyValue& oldValue = property->GetValue();
    oldValue.ClearList();

    for (
      list<wxString>::iterator iString = pStringList->begin();
      iString != pStringList->end();
      ++iString)
    {
      const wxString& s = *iString;
...
class wxPropertyStringListEditorDialog: public wxDialog
{
  public:
    ...
    wxPropertyStringListEditorDialog(
      ...
      const wxString& name = wxT("stringEditorDialogBox"))
    ...
    virtual ~wxPropertyStringListEditorDialog()
    {
    }
...
{
  public:
    wxPropertyStringListEditorText(
      wxWindow *parent,
      wxWindowID id,
      const wxString& val,
      const wxPoint& pos = wxDefaultPosition,
  ...
  void OnKillFocus()
  {
    wxPropertyStringListEditorDialog *dialog =
      (wxPropertyStringListEditorDialog *)GetParent();
...
bool wxListOfStringsListValidator::EditStringList(
  wxWindow *parent,
  list<wxString>* pStringList,
  const wxChar *title)
{
  ...
  wxBeginBusyCursor();
  wxPropertyStringListEditorDialog *dialog =
    new wxPropertyStringListEditorDialog(
      parent,
      title,
      wxPoint(10, 10),
      wxSize(400, 400));

  dialog->m_stringList = pStringList;

  dialog->m_listBox = new wxListBox(
    dialog,
  ...
  dialog->m_stringText = new wxPropertyStringListEditorText(
    dialog,
    wxID_PROP_SL_TEXT,
    wxEmptyString,
    wxPoint(5, 240),
  ...
  for (
    ...
    ++iString)
  {
    const wxString& str = *iString;

    list<wxString>::iterator* piStringCopy =
      new list<wxString>::iterator(iString);

    // Save node as client data for each listbox item
    dialog->m_listBox->Append(str, (void *)piStringCopy);
  }
...
void wxPropertyStringListEditorDialog::OnAdd(wxCommandEvent& event)
{
  ...
  list<wxString>::iterator* piLast =new list<wxString>::iterator(
    m_stringList->insert(m_stringList->end(), initialText));
  m_listBox->Append(initialText, (void *)piLast);
  m_currentSelection = m_stringList->size() - 1;
...
void
wxPropertyStringListEditorDialog::OnCloseWindow(wxCloseEvent& event)
{
  SaveCurrentSelection();
