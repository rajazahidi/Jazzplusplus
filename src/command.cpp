...
void tCmdConvertToModulation::ExecuteTrack(tTrack *t)
{
  ...
  long pitchsteparray[] = {0x0000, 0x07ff, 0x1000, 0x17ff, 0x2000, 0x27ff, 0x3000, 0x37ff,  0x3fff};
...
void tCmdMidiDelay::ExecuteEvent(tTrack *t, tEvent *e)
{
  tKeyOn *k;
  ...
  for(int i=1; i< repeat; i++)
	...
	k->Veloc=(uchar)(pow(scale,i)*k->Veloc);
	t->Put(k);
      }
    }
...
tCmdMapper::tCmdMapper(tFilter *f, prop src, prop dst, tRndArray &arr, int nb, int ad)
  : tCommand(f),
    array(arr)
{
  n_bars          = nb;
...
void tCmdMapper::ExecuteEvent(tTrack *t, tEvent *e)
{
  tKeyOn *k = e->IsKeyOn();
  if (k)
  {
